/* 	Author:			Anthony Lucchese
	Program Name:		asgn7-lucchesea2.c
	Compile and run:	gcc -o asgn7 asgn7-lucchesea2.c -Wall -pthread
	...
	while (1) {
		...
		if (listPtr->numOfJobs > 0) {
			...
			if (currentSysTime >= nextJob->submissionTime + nextJob->startTime) {
				...
				pthread_create(&forkCall, NULL, createFork, (void*)tempJob);
				pthread_join(forkCall, NULL);
				//createFork(tempJob);
...
int main() {
	List *listPtr;
	listPtr = createList();
	pthread_t disp, sched;

	pthread_create(&disp, NULL, schedulerThread,(void*)listPtr);	
	pthread_create(&sched, NULL, dispatcherThread, (void*)listPtr);
